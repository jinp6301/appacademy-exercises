...
class ComputerPlayer
  ...
  def candidate_words(word_length)
    dict = dictionary
    dictionary.select {|word| word.length == word_length }
  end

  def reduce_word_list(guess, guess_index)
    word_list.each do |word|
      word.split(//).each_with_index do |char, index|
        if char == guess && !guess_index.include? index
          *delete fucntion
        elsif char != guess && guess_index.include? index
          *delete
        end
      end
    end
  ...
  def remove_candidate_words_with_letters(word_list, letter)
    new_word_list = []
    unless letter.nil?
        word_list.each do |word|
    ...
    unless index.nil? || letter.nil?
      ...
      index.each do |i|
        ...
        words_of_correct_length.each do |word|
          inner_word_list_with_restrictions << word if word[i] == letter
        end
